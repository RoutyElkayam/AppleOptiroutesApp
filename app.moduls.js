    ...
    .run([
      ...
      function (
        ...
        $rootScope.manageErrorSigningPOD = function(order){
          ...
          var orderIndex = $filter('getByIdFilter')($rootScope.nonactiveRoutes, parseInt(order_id));
          if(orderIndex>-1 && $rootScope.nonactiveRoutes[orderIndex]){
            $rootScope.nonactiveRoutes[orderIndex].status = 4;
          }
        ...
        $rootScope.$on('setSignatureOk', function (event, response) {
          // response.config.url contains "...order/set_signature&id=123&images=..."
          // response.data contains server payload
          console.log("set_signature OK:", response);
        
          // example: get order id from url (same style you do in httpError)
          var order_id = (response.config.url.split('&'))[1].slice(3);
        
          // delete draft here (call your draft service / localStorage cleanup / etc)
          PodDraftStore.clear(order_id);
        });
        ...
        //Prevent reloading the app during offline mode
        // Block browser refresh/close while offline (shows native prompt)
        $window.addEventListener('beforeunload', function (e) {
            if (!$rootScope.isOnline) {
              e.preventDefault();
              e.returnValue = ''; // required for the native confirmation dialog
            }
        });
        ...
        $rootScope.arraylang = {
          ...
          no_pod_file: {
              en: "No POD file for order number: ",
              he: "לא מצאנו קובץ תעודת משלוח עבור הזמנה מספר "
          },
          please_manual_signing: {
              en: ". Please perform manual signing.",
              he: ". נא החתם ידנית."
          },
          order_prefix: {
              en: "order number: ",
          ...
          and_order: {
              en: ", and for order number ",
              he: " , וכן עבור הזמנה מספר "
          },
          orders:{
            he: "הזמנות",
            en: "Orders",
          ...
          camera_not_ready:{
            he: "המצלמה עדיין לא מוכנה, נא ללחוץ שוב בעוד רגע",
            en: "The camera is not ready yet, please wait",
          },
          photo_captured:{
            he: "התמונה צולמה בהצלחה",
            en: "Photo captured successfully",
          },
          max_images_reached:{
            he: "מספר התמונות  שניתן לצרף להוכחת מסירה מוגבל ל-3",
            en: "The number of images can be attached to the delivery proof is limited to 3",
          },
          ...
          $.ajax({
            ...
            function (response) {
              ...
              if (response && response != "null" && response != null) {
                ...
                $rootScope.remark_options = Array.isArray(data.remark_options)
                    ? data.remark_options.sort((a,b) =>
