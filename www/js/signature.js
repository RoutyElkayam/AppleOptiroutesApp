...
angular.module('signature').directive('signaturePad', ['$interval', '$timeout', '$window',
  function ($interval, $timeout, $window) {
    ...
    return {
      restrict: 'EA',
      replace: true,
      template: '<div class="signature col-xs-12 no-padding" style="width: 100%; max-width:{{width}}px; height: 100%; max-height:{{height}}px;"><canvas class="col-xs-12 no-padding" style="display: block; margin: 0 auto;" height="200"  ng-mouseup="onMouseup()" ng-mousedown="notifyDrawing({ drawing: true })"></canvas></div>',
      scope: {
        accept: '=?',
        clear: '=?',
        disabled: '=?',
