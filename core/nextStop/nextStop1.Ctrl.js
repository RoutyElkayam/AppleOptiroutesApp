angular.module("RouteSpeed.nextStop").controller("nextStopCtrl", [
  ...
  function nextStopCtr(
    ...
    // Helper function to request camera and necessary Android permissions
    function requestCameraPermissions(successCallback, errorCallback) {
      if (!window.cordova) {
        // Not in Cordova environment, proceed directly
        successCallback();
        return;
      }
      ...
      if (cordova.plugins && cordova.plugins.permissions) {
        ...
        cordova.plugins.permissions.checkPermission(
              ...
              cordova.plugins.permissions.requestPermission(
                ...
                function (requestStatus) {
                  if (requestStatus.hasPermission) {
                    ...
                    if (errorCallback) {
                      errorCallback("Camera permission denied. Please enable camera access in app settings.");
                    } else {
                      alert("Camera permission denied. Please enable camera access in app settings.");
                    }
                  }
                },
                function () {
                  ...
                  if (errorCallback) {
                    errorCallback("Failed to request camera permission");
                  } else {
          ...
          function () {
            // If permission check fails (older Android versions), proceed anyway
            successCallback();
          }
        );
      } else {
        // Permissions plugin not available, proceed directly (will fail gracefully if no permission)
        successCallback();
      }
    }
    ...
    $scope.beforeDone = function (status) {
      ...
      if(navigator.onLine && ($scope.double_documentaion || ($scope.current_stop.status != 0 && $scope.current_stop.status != 4))){
